...
int init_main_screen(Server *server)
{
    ...
    server->main_screen.window = newwin(server->main_screen.win_width, server->main_screen.win_height,
                                        server->main_screen.upper_y, server->main_screen.upper_x);
...
int init_stats_screen(Server *server)
{
    ...
    server->stats_screen.window = newwin(server->stats_screen.win_width, server->stats_screen.win_height,
                                         server->stats_screen.upper_y, server->stats_screen.upper_x);
...
int init_err_panel(Server *server)
{
    ...
    server->err_panel.window = newwin(server->err_panel.win_width, server->err_panel.win_height,
                                      server->err_panel.upper_y, server->err_panel.upper_x);
...
void print_stats_screen(Server *server)
{
    ...
    for (int i = 0; i < PLAYERS; i++)
    {
        ...
        {
            sprintf(temp, "%d", i + 1);
            mvwprintw(server->stats_screen.window, start_y + 3, start_x + 1, "     ");
            mvwprintw(server->stats_screen.window, start_y + 7, start_x, "     ");

            mvwaddch(server->stats_screen.window, start_y, start_x - 1,
                     server->players[i].packet.usr | COLOR_PAIR(server->players[i].packet.stats.color));
            mvwprintw(server->stats_screen.window, start_y, start_x, "Player");
            mvwprintw(server->stats_screen.window, start_y, start_x + 6, temp);
            mvwprintw(server->stats_screen.window, start_y + 1, start_x, "       ");
            mvwprintw(server->stats_screen.window, start_y + 1, start_x, "%d", server->players[i].user_pid);
...
void setDroppedTreasure(Server *server, Player *player1, Player *player2)
{
    ...
    mvwaddch(server->main_screen.window, player1->packet.stats.pos_y, player1->packet.stats.pos_x,
             'D' | COLOR_PAIR(TREASURE));
    ll_push_back(server->dropped_treasures, player1->packet.stats.pos_x, player1->packet.stats.pos_y,
                 player1->packet.stats.carried_coins + player2->packet.stats.carried_coins);
...
void printPlayers(Server *server)
{
    ...
    {
        if (server->players[i].active == -1) continue;
        mvwaddch(server->main_screen.window, server->players[i].packet.stats.pos_y,
                 server->players[i].packet.stats.pos_x,
                 server->players[i].packet.usr | COLOR_PAIR(server->players[i].packet.stats.color));
    }
    for (int i = 0; i < BEASTS; i++)
    {
        if (server->beasts[i].active == -1) continue;
        mvwaddch(server->main_screen.window, server->beasts[i].packet.stats.pos_y, server->beasts[i].packet.stats.pos_x,
                 server->beasts[i].packet.usr | COLOR_PAIR(server->beasts[i].packet.stats.color));
    }
}
        ...
        if (move == KEY_UP)
        {
            if (legalMove(server->main_screen.window, player->packet.stats.pos_x, player->packet.stats.pos_y - 1))
            {
                ...
                if ((ret = isTreasure(server, player->packet.stats.pos_x, player->packet.stats.pos_y - 1)) > 0)
                    increaseCoins(player, ret, player->packet.stats.pos_x, player->packet.stats.pos_y - 1,
                                  server->dropped_treasures);
        ...
        } else if (move == KEY_LEFT)
        {
            if (legalMove(server->main_screen.window, player->packet.stats.pos_x - 1, player->packet.stats.pos_y))
            {
                ...
                if ((ret = isTreasure(server, player->packet.stats.pos_x - 1, player->packet.stats.pos_y)) > 0)
                    increaseCoins(player, ret, player->packet.stats.pos_x - 1, player->packet.stats.pos_y,
                                  server->dropped_treasures);
...
void printMap(UserPacket packet, char *map_snippet)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        for (int j = 0; j < 6; j++)
        {
            ...
            else if (map_snippet[iter] == 'c' || map_snippet[iter] == 'T' || map_snippet[iter] == 't' ||
                     map_snippet[iter] == 'D')
